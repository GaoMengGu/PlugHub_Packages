...
模板可直接使用当前 Revit 项目`管理 > 项目参数`中显示的任意可绑定类别名称，例如地形、墙、门、窗、楼板、结构柱、风管、管道和电缆桥架等。插件运行时从当前项目的 `Settings.Categories` 解析类别，仅排除 Revit 本身不允许绑定项目参数的类别；原有`结构墙`、`结构楼板`等兼容别名仍可使用。类别只在选定项目后校验，因此错误会按模板行号提示。
