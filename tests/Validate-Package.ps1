...
Require-Text "src\PlugHub.ClearHeightAnalysis\Services\ObstacleCollector.cs" "ObstacleVerticalFilter.ShouldIncludeOverheadObstacle" "Clear height current-floor vertical filter"
Require-Text "src\PlugHub.ClearHeightAnalysis\ClearHeightAnalysisCommand.cs" "MaximumGridCells" "Clear height grid count guard"
Require-Text "src\PlugHub.ClearHeightAnalysis\ClearHeightAnalysisCommand.cs" "HeatmapResultLimiter.LimitForRendering" "Clear height heatmap render limiter"
Require-File "src\PlugHub.ClearHeightAnalysis\Services\ObstacleVerticalFilter.cs"
Require-File "src\PlugHub.ClearHeightAnalysis\Services\HeatmapResultLimiter.cs"
Reject-Text "src\PlugHub.ClearHeightAnalysis\Services\ObstacleCollector.cs" "ReferenceIntersector" "Clear height must not rely on ray-only sampling"
Reject-Text "src\PlugHub.ClearHeightAnalysis\Services\ObstacleCollector.cs" "FindReferencesByDirection" "Clear height must not rely on ray-only sampling"
