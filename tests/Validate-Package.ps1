...
Require-Text "src\PlugHub.HubeiReportParameters\HubeiReportSelectionForm.cs" "写入模板真实数据" "Hubei report optional actual data write"
Require-Text "src\PlugHub.HubeiReportParameters\HubeiReportSelectionForm.cs" "未勾选时仅使用模板默认值" "Hubei report actual data fallback hint"
Require-Text "src\PlugHub.HubeiReportParameters\HubeiReportSelectionForm.cs" "_writeActualValuesCheckBox.Checked = true" "Hubei report actual data default selection"
...
Require-Text "src\PlugHub.HubeiReportParameters\SyncHubeiReportParametersCommand.cs" "writeActualValues && !string.IsNullOrWhiteSpace(definition.ActualValue)" "Hubei report optional actual value gating"
Require-Text "src\PlugHub.HubeiReportParameters\HubeiReportTemplate.cs" "ValidateDuplicateNames(template.Rows, writeActualValues)" "Hubei report value conflict validation follows actual data option"
